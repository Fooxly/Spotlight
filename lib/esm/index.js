...
export function Spotlight({ isDarkMode, showRecentlyUsed = 5, showTips = true }) {
    ...
    return (_jsxs(ThemeProvider, Object.assign({ theme: calculatedTheme }, { children: [_jsx(SpotlightComponent, { showTips: showTips }), _jsx(Toast, {})] })));
}
...
export function toast(message) {
    const handleRequest = (ev) => {
        ...
        document.removeEventListener(TOAT_EVENT_KEY, handleRequest);
    };
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    document.addEventListener(TOAT_EVENT_KEY, handleRequest);
