...
export function Result({ hasIcons, item, index, selected, onSoftSelect, onSelect }) {
    const enableFocus = () => onSoftSelect(index);
    const Icon = getCommandIcon(item.options?.icon);
    const handleAction = () => {
        onSelect(item);
    };
    return (_jsxs(Container, { id: `command-${index}`, "$selected": selected, onClick: handleAction, onMouseMove: enableFocus, onFocus: enableFocus, children: [_jsxs(Left, { children: [hasIcons && (_jsx(IconWrapper, { children: !!Icon && _jsx(Icon, { size: 24, color: 'gray4' }) })), _jsx(Title, { children: item.title })] }), selected && (_jsx(Type, { children: item.type === 'command' ? 'Run command' : 'Jump to' }))] }));
}
