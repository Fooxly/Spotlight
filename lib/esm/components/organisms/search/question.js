...
import { getUUID, PICKED_RESULT_EVENT_KEY, QUESTION_EVENT_KEY, QUESTION_RESULT_EVENT_KEY, SEARCH_CLOSED_EVENT_KEY, useSearchContext, } from '../../../utils';
import './styles.css';
export function Question() {
    const { type, results, setType, setSearch, setVisible, setResults, setShowIcons, setPlaceholder, setSelectedItem, } = useSearchContext();
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
        document.addEventListener(SEARCH_CLOSED_EVENT_KEY, handleQuestionEnd);
        return () => {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
            document.removeEventListener(QUESTION_EVENT_KEY, handleQuestionStart);
            // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
            document.removeEventListener(SEARCH_CLOSED_EVENT_KEY, handleQuestionEnd);
        };
