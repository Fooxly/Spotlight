...
import { filterResults, executeItem, updateHistory, clearHistory, ERRORS, INPUT_TYPE_EVENT_KEY, TEXT_INPUT_RESULT_EVENT_KEY, SpotlightContext, PAGES, COMMANDS, UPDATE_SPOTLIGHT_EVENT_KEY, } from '../../utils';
...
export function SpotlightComponent({ showTips }) {
    var _a;
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
        document.removeEventListener(UPDATE_SPOTLIGHT_EVENT_KEY, forceUpdateSpotlight, false);
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
        document.addEventListener(UPDATE_SPOTLIGHT_EVENT_KEY, forceUpdateSpotlight, false);
            ...
            return filterResults(search, {
                ...
                items: answers.map((answer) => {
                    ...
                    return ({
                        ...
                        title: typeof answer === 'string' ? answer : (_a = answer.title) !== null && _a !== void 0 ? _a : answer.label,
                        returnKey: typeof answer === 'string' ? answer : (_b = answer.title) !== null && _b !== void 0 ? _b : answer.key,
                        options: {
                            icon: typeof answer === 'string' ? null : answer.icon,
                            keywords: typeof answer === 'string'
                                ? null
    ...
    const onSectionAction = useCallback((type) => {
        if (type === 'pages') {
            setSubMenuItem({
                title: 'All Pages',
                action: (page) => {
    ...
    const sectionActionText = useCallback((type) => {
        switch (type) {
    ...
    return ReactDOM.createPortal(!visible ? null : (_jsx(Container, Object.assign({ id: 'fooxly-spotlight' }, { children: _jsxs(SpotlightContext.Provider, Object.assign({ value: {
                type: spotlightType,
                            ...
                            indexedResults[0].results.length > 0 && (_jsxs(Results, Object.assign({ ref: resultsRef }, { children: [!(search === null || search === void 0 ? void 0 : search.length) && !subMenuItem && spotlightType === 'search' && showTips && (_jsx(Tip, { dangerouslySetInnerHTML: { __html: (_a = decodeHTML(activeTip)) !== null && _a !== void 0 ? _a : '' } })), indexedResults.map((category) => (_jsx(Section, { title: category.title, results: category.results, showIcons: resultsHaveIcons, selectedIndex: selectedIndex, 
                                    // eslint-disable-next-line react/jsx-handler-names
