...
export function SpotlightComponent() {
    ...
    return ReactDOM.createPortal(!visible ? null : (_jsxs(Container, { id: 'fooxly-spotlight', children: [_jsx(Background, { onClick: hideSpotlight }), _jsxs(Content, { children: [_jsx(SearchInput, { hasResults: !!indexedResults?.length || !!error, placeholder: subMenuItem ? 'Choose an option...' : 'Search or jump to...', value: search, loading: loading, fref: inputRef, 
                        // eslint-disable-next-line react/jsx-handler-names
                        onChange: setSearch }), !!error && (_jsx(Error, { message: error, onDismiss: () => setError('') })), indexedResults.length > 0 && indexedResults[0].results.length > 0 && (_jsx(Results, { ref: resultsRef, children: indexedResults.map((category) => (_jsx(Section, { title: category.title, results: category.results, showIcons: resultsHaveIcons, selectedIndex: selectedIndex, 
                            // eslint-disable-next-line react/jsx-handler-names
