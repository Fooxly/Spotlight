...
function SearchBase() {
    ...
    const results = (0, react_1.useMemo)(() => {
        return (0, search_1.getResultsByParentId)(utils_1.catalog.items, parentId);
    ...
    (0, react_hotkeys_hook_1.useHotkeys)('backspace', (ev) => {
        if (search.length > 0 || !parentId)
            return;
        preventDefault(ev);
        const parent = (0, search_1.getResultById)(utils_1.catalog.items, parentId);
    ...
    (0, react_hotkeys_hook_1.useHotkeys)('enter', (e) => {
        ...
        if (type === 'input') {
            ...
            if (!selectedItem)
                return;
            const item = (0, search_1.getResultById)(utils_1.catalog.items, selectedItem);
            if (!item)
                return;
