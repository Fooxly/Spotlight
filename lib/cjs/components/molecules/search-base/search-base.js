...
function SearchBase() {
    const { type, catalog, visible, parentId, selectedItem, setVisible, setLoading, setParentId, setSelectedItem, } = (0, utils_1.useSearchContext)();
    const searchRef = (0, react_1.createRef)();
    const resultsRef = (0, react_1.createRef)();
    const [search, setSearch] = (0, react_1.useState)('');
    const results = (0, react_1.useMemo)(() => {
        return (0, search_1.getResultsByParentId)(catalog, parentId);
    }, [catalog, parentId]);
    ...
    (0, react_hotkeys_hook_1.useHotkeys)('backspace', (ev) => {
        if (search.length > 0 || !parentId)
            return;
        preventDefault(ev);
        const parent = (0, search_1.getResultById)(catalog, parentId);
    ...
    (0, react_hotkeys_hook_1.useHotkeys)('enter', (e) => {
        ...
        if (type === 'input') {
            ...
            if (!selectedItem)
                return;
            const item = (0, search_1.getResultById)(catalog, selectedItem);
            if (!item)
                return;
