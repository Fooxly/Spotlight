...
function filterResults(searchText, menu) {
    ...
    if ((text === null || text === void 0 ? void 0 : text.length) && text.charAt(0) === '>' && !menu) {
        if (text.length === 1) {
            return sortResults(constants_1.COMMANDS, false, false);
        }
        const fuse = new fuse_js_1.default(constants_1.COMMANDS, FUSE_PROPS);
        ...
        if (text.length === 1) {
            return sortResults(constants_1.PAGES, false, false);
        }
        const fuse = new fuse_js_1.default(constants_1.PAGES, FUSE_PROPS);
        return sortResults(fuse.search(searchText).map((result) => result.item));
    }
...
function createCategory(title, items, options) {
    ...
    if ((options === null || options === void 0 ? void 0 : options.type) !== 'history') {
    ...
    const mappedItems = sortedItems
        .map((item, index) => { var _a; return ({ item, index: index + ((_a = options === null || options === void 0 ? void 0 : options.indexOffset) !== null && _a !== void 0 ? _a : 0) }); });
    // Place the icon list command at the bottom of the list
    const iconListCommandIndex = mappedItems
